...
Deno.test({
  ...
  fn: async (): Promise<void> => {
    const foundMoveResponse = await move([{
      name: "name",
      value: "transform",
      type: 3,
    }]);
...
Deno.test({
  ...
  fn: async (): Promise<void> => {
    const first = await move([{
      name: "name",
      value: "dont you know who i am",
      type: 3,
    }]);
    assertExists(first.data.embeds);
    assertExists(first.data.embeds[0].title);
    const other = await move([{
      name: "name",
      value: "Don't You know WHO I AM",
      type: 3,
    }]);
...
Deno.test({
  name: "Move not found",
  fn: async (): Promise<void> => {
    const notFoundMoveResponse = await move([{
      name: "name",
    ...
    assertEquals(
      notFoundMoveResponse.data.components[0].components.length <= 5,
      true,
    );
  },
});
